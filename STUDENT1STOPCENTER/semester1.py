    ...
    try:
        ...
        for i, (course_code, course_name) in enumerate(course1_dict.items()):
            ...
            if credit_hour and grade:
                ...
                mycursor.execute("INSERT INTO `grade_semester1` (student_id, course_code, credit_hours, grade) VALUES (%s, %s, %s, %s)",
        ...
        if total_credit_hours > 0:
            ...
            mycursor.execute("UPDATE grade_semester1 SET gpa = %s WHERE student_id = %s", (gpa, student_id))
            mydb.commit()
            #messagebox.showinfo("Data Saved Successfully .")
        else:
            messagebox.showwarning("Input Error", "Please enter valid credit hours and grades.")
    ...
    try:
        ...
        for i, (course_code, course_name) in enumerate(course1_dict.items()):
            ...
            if credit_hour and grade:
                ...
                mycursor.execute("UPDATE grade_semester1 SET credit_hours = %s, grade = %s WHERE student_id = %s AND course_code = %s",
...
def delete_data():
    try:
        ...
        mycursor.execute("DELETE FROM grade_semester1 WHERE student_id = %s", (student_id,))
        mydb.commit()
...
# saving student info
grade_info_frame = tk.LabelFrame(frame, text='Student Grade Information (Semester 1)',bg='lightyellow' )
...
# Message Label
message_label = tk.Label(frame, bg='yellow')
