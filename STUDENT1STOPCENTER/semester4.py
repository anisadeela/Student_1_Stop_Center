...
def calculate_gpa():
    try:
        ...
        for i in range(5):  # Assuming there are 5 courses
...
# Function to insert or update data into MySQL database
def modify_data(action):
    try:
        ...
        # Add the 'credit_hours' column if not present in the table
        mycursor.execute("SHOW COLUMNS FROM grade_semester4 LIKE 'credit_hours'")
        result = mycursor.fetchone()

        if result is None:
            mycursor.execute("ALTER TABLE grade_semester4 ADD COLUMN credit_hours DECIMAL(5,2)")
        ...
        for i, (course_code, course_name) in enumerate(course1_dict.items()):
            ...
            if credit_hour and grade:
                ...
                if action == 'insert':
                    ...
                    mycursor.execute("INSERT INTO grade_semester4 (student_id, course_code, credit_hours, grade) VALUES (%s, %s, %s, %s)",
                                     (student_id, course_code, credit_hour, grade))
                elif action == 'update':
                    # Update credit hours and grades in the database
                    mycursor.execute("UPDATE grade_semester4 SET credit_hours = %s, grade = %s WHERE student_id = %s AND course_code = %s",
        ...
        if total_credit_hours > 0:
            ...
            mycursor.execute("UPDATE grade_semester4 SET gpa = %s WHERE student_id = %s", (gpa, student_id))
            mydb.commit()
...
def delete_data():
    try:
        ...
        mycursor.execute("DELETE FROM grade_semester4 WHERE student_id = %s", (student_id,))
        mydb.commit()
...
# saving student info
grade_info_frame = tk.LabelFrame(frame, text='Student Grade Information (Semester 4)', bg='lightyellow')
...
# Save button
save_button = tk.Button(button_frame, text='Save', command=lambda: modify_data('insert'), bg='lightgrey')
save_button.grid(row=1, column=1, padx=5, pady=5, sticky="ew")

# Update button
update_button = tk.Button(button_frame, text='Update', command=lambda: modify_data('update'), bg='lightgrey')
