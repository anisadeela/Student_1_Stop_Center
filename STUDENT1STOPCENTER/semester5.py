    ...
    try:
        ...
        for i, (course_code, course_name) in enumerate(course1_dict.items()):
            ...
            if credit_hour and grade:
                ...
                mycursor.execute("INSERT INTO `grade_semester5` (student_id, course_code, credit_hours, grade, gpa) VALUES (%s, %s, %s, %s, %s)",
                                 (student_id, course_code, credit_hour, grade, 0))  # Assuming default GPA is 0
                mydb.commit()
        ...
        if total_credit_hours > 0:
            ...
            mycursor.execute("UPDATE grade_semester5 SET gpa = %s WHERE student_id = %s", (gpa, student_id))
            mydb.commit()
    ...
    try:
        ...
        for i, (course_code, course_name) in enumerate(course1_dict.items()):
            ...
            if credit_hour and grade:
                ...
                mycursor.execute("UPDATE grade_semester5 SET credit_hours = %s, grade = %s WHERE student_id = %s AND course_code = %s",
...
def delete_data():
    try:
        ...
        mycursor.execute("DELETE FROM grade_semester5 WHERE student_id = %s", (student_id,))
        mydb.commit()
...
# saving student info
grade_info_frame = tk.LabelFrame(frame, text='Student Grade Information (Semester 5)', bg='lightyellow')
